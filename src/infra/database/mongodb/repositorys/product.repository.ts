...
export class ProductRepository implements iProductRepository {
  constructor(private readonly database: Db, private readonly colletion: Collection<ProductEntity>) {}
  ...
  async productOutput(productDetails: TransactionEntity.ProductIncomingTransaction, options?: iProductRepository.Options): Promise<ProductEntity> {
  ...
  isDuplicatedProduct(produt: Partial<ProductEntity>, options?: iProductRepository.Options): Promise<ProductEntity> {
  ...
  async archiveProduct(productId: string, options?: iProductRepository.Options): Promise<boolean> {
    const result = await this.colletion.updateOne({ id: productId }, { $set: { archived_date: new Date() } }, { session: options?.session.get() });
  ...
  async unarchiveProduct(productId: string, options?: iProductRepository.Options): Promise<boolean> {
    const result = await this.colletion.updateOne({ id: productId }, { $set: { archived_date: null } }, { session: options?.session.get() });
    if (result.modifiedCount > 0) {
      return true;
    }
    return false;
  ...
  private findOneWithProjection(filter: Filter<ProductEntity>, options?: iProductRepository.Options): Promise<ProductEntity> {
